...
let chartData = { labels: [], datasets: [{ label: 'Speed', data: [], borderColor: '#00ff9f', backgroundColor: 'rgba(0, 255, 159, 0.1)', fill: true, tension: 0.4, pointRadius: 0 }] };
let peakSpeed = 0;
...
function initChart() {
    ...
    speedChartInstance = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                y: { beginAtZero: true, grid: { color: 'rgba(255, 255, 255, 0.05)' }, ticks: { color: '#8b949e', font: { family: "'Roboto Mono', monospace" } } },
                x: { grid: { display: false }, ticks: { color: '#8b949e', maxTicksLimit: 8, font: { family: "'Roboto Mono', monospace" } } }
            },
    ...
    try {
        ...
        if (data.success) {
            if (type === 'target') {
                ...
                logToTerminal(`Target payload loaded: ${file.name}`, 'success');
            } else {
                state.wordlistFile = data;
                updateWordlistFileInfo(data);
                Toast.success('Wordlist uploaded successfully');
                logToTerminal(`Dictionary matrix loaded: ${file.name}`, 'success');
...
async function checkDependencies() {
    try {
        ...
        if (data.all_installed) {
            ...
            logToTerminal('All system dependencies verified.', 'success');
        } else {
            ...
            elements.mainContent.style.display = 'block';
            logToTerminal('Missing system dependencies detected.', 'warning');
        }
    } catch (error) {
...
elements.startCrack.addEventListener('click', async () => {
    ...
    try {
        ...
        if (data.success) {
            ...
            logToTerminal('Initiating brute-force sequence...', 'warning');
        } else {
...
async function checkJobStatus() {
    ...
    try {
        ...
        if (job.status === 'completed') {
            ...
            logToTerminal(`Decryption successful. Password: ${job.result.password}`, 'success');
        } else if (job.status === 'failed' || job.status === 'stopped') {
            clearInterval(state.statusCheckInterval);
            showError(job.error || 'Cracking failed');
            logToTerminal(`Process ${job.status}: ${job.error || ''}`, 'error');
        }
    } catch (error) {
...
function updateProgress(job) {
    ...
    if (elements.currentMethod) elements.currentMethod.textContent = job.current_method || '-';
    elements.progressText.textContent = `${job.processed.toLocaleString()}/${job.total.toLocaleString()} (${Math.round(job.progress)}%)`;
    
    const currentSpeed = job.speed || 0;
    if (elements.speedText) {
        elements.speedText.textContent = `${currentSpeed.toLocaleString()} p/s`;
        updateChart(currentSpeed);
        
        if(currentSpeed > peakSpeed) {
            peakSpeed = currentSpeed;
            const peakEl = document.getElementById('metricPeakSpeed');
            if (peakEl) peakEl.textContent = `${peakSpeed.toLocaleString()} p/s`;
        }
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    initChart();
    logToTerminal('Initializing CyberMatrix UI...', 'info');
    setupFileUploads();
    checkDependencies();
});